// Service Worker — injects DOM patch before async Next.js bundles.
...
// Skip waiting so the new SW takes over immediately on activation.
self.addEventListener('install', () => self.skipWaiting());

// Claim all existing clients so this SW controls pages immediately.
...
async function transformNavigation(request) {
  ...
  // Stream the response, injecting our patch right after the opening <head> tag.
  // The opening <head> always appears in the very first chunk of a Next.js response.
  let injected = false;
  ...
  const transform = new TransformStream({
    transform(chunk, controller) {
      ...
      if (text.includes('<head>')) {
        const modified = text.replace('<head>', '<head>' + PATCH);
  ...
  // Build new headers without Content-Length (body size changes after injection).
